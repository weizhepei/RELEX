...
def call_with_timeout(func, *args, timeout=1):
    """Thread-based timeout wrapper. Uses threads instead of processes to avoid
    CUDA fork issues and system process limits."""
    output_queue = multiprocessing.Queue()
    with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
        future = executor.submit(func, *args, output_queue=output_queue)
        try:
